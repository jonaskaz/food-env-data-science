...
The most obvious thing to notice from is the fact that this dataset includes a lot of different statistics. Specifically, there are 311 statistics reported for 3140 FIPS codes. We also looked at some summary statistics for `PCT_DIABETES_ADULTS08` which contains the percentage rates of diabetes diagnosed in adults in each county. We can see that the mean diabetes rate is about 9.9%, which is slightly less than the CDC's estimate of national diabetes rate at 10.2% [6]. A histogram allows us to visualize the distribution that diabetes rate takes across all counties. The distribution looks roughly normal, but the peak is slightly shifted to the left. Interestingly, the bin at 17% appears to be *slightly* higher than the surrounding bins. Furthermore, when we isolate the counties with the ten highest diabetes rates, we find that they are all in Alabama or Georgia. Eight out of ten are counties in Alabama, while the other two are in Georgia. 
